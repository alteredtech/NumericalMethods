...
def pathcontroller(bot):
  (en,eh,kff,dl) = bot['path'].nearest( \
    bot['state']['x'], bot['state']['y'], bot['state']['yaw'])
  kd = 0.2*en + 1*eh + kff;
...
def plant(t,z,bot):
  setstate(bot,z)
  speed = math.sqrt(bot["state"]["vx"]*bot["state"]["vx"] + bot["state"]
["vy"]*bot["state"]["vy"])
  cq = math.cos(bot["state"]["yaw"])
  sq = math.sin(bot["state"]["yaw"])
...
def simulate(tstart, tend, h, bot, goal):
  ...
  with open('out.txt','w') as w:
    for ii in range(n):
      ...
      pathcontroller(bot)
      v = math.sqrt(bot['state']['vx']*bot['state']['vx'] + bot['state']
['vy']*bot['state']['vy'])
      w.write("{} {} {} {} {} {}\n".format(t,bot['state']['x'],bot['state']
['y'],v,bot['wheel_torque'],bot['steer_angle']))
      # print('bot')
...
if __name__ == '__main__':
  ...
  with open('data.json','r') as r:
    ...
    dat["bot"]["path"] = SplinedPath(dat["x"],dat["y"])
    goal = {"x":-40, "y":10}
    simulate(dat["t0"], dat["t1"], dat["dt"], dat["bot"], goal)
